...
mod response_stream_repro_tests {
    ...
    #[test]
    fn streaming_response_activity_blocks_idle_eviction_until_body_closes() {
        let mut state = idle_state(1_000);

        // A routed request remains active until its response body closes. SSE
        // chunks do not need to refresh the idle lease: the open request is a
        // stronger pin and makes the cell ineligible for ordinary eviction.
        state.activate_request(7, "Sse:one".into());
        ...
        // Closing the body releases the activity. The next idle interval can
        // then select the cell normally.
        let _ = on_event(&mut state, Event::ActivityFinished { request: 7 });
        let effects = on_event(
            &mut state,
            Event::LoadSampled {
                load: pressure::Load {
        ...
        assert!(
            ...
            "a closed response kept the cell pinned"
        );
    }
}
